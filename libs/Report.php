		...
		if (count($data) !== 0 ) {
			...
			foreach ($data as $row_data) {
				...
				foreach ($row_data as $key => $value) {
                                    ...
                                    // If the data array line starts with "<B>", Bold this row.
                                    if($row_data[0] == "<B>") {
                                        if($column == 0) {
                                            
                                        } else {
                                        
                                        $excel_file->getActiveSheet()->setCellValueByColumnAndRow($column-1,$start_row,$value);
                                        $excel_file->getActiveSheet()->getStyleByColumnAndRow($column-1,$start_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                                        $excel_file->getActiveSheet()->getStyleByColumnAndRow($column-1,$start_row)->getFont()->setBold(true);
                                        $excel_file->getActiveSheet()->getColumnDimensionByColumn($column-1)->setAutoSize(true);
                                        }
