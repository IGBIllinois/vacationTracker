...
if(isset($_POST['selectShareUser']))
{
	...
	if($userToShareWith)
	{
		...
		$sqlDataBase->insertQuery($queryInsertShare);
	}
}
	...
	<table class="content" height="100%">
		...
		<tr>
				...
				<table width="100%">
						...
						<td colspan=2>
							<div id="view_calendar">
								...
								}else
								{
									...
									echo " onclick=\"this.form.submit();\" ></td><td><label for=\"calendarchecbox_".$loggedUser->getUserId()."\">".$loggedUser->getFirstName()." ".$loggedUser->getLastName()."</label></td></tr>";
								}
								if(isset($sharedCalendars))
								{
									foreach($sharedCalendars as $id=>$calendarUser)
									{
										echo "<tr><td><input type=\"checkbox\" name=\"employeeids[]\" value=".$calendarUser['user_id']." id=\"calendarchecbox_".$calendarUser['user_id']."\"";
					...
					<tr>
						<td class="form_field" valign="top" colspan=2>
							...
							<?php
							$queryAllUnsharedUsers = "SELECT user_id, first_name, last_name FROM users WHERE enabled=".ENABLED." AND user_id NOT IN (SELECT viewer_id FROM shared_calendars WHERE owner_id=".$loggedUser->getUserId().") ORDER BY first_name ASC";
							$allUnsharedUsers = $sqlDataBase->query($queryAllUnsharedUsers);
							...
							<div id="share_calendar">
								<table width="100%">
								<?php
								$querySharedUsers = "SELECT u.user_id, u.first_name, u.last_name FROM users u, shared_calendars sc WHERE u.user_id = sc.viewer_id AND sc.owner_id=".$loggedUser->getUserId()." ORDER BY u.first_name ASC";
								$sharedUsers = $sqlDataBase->query($querySharedUsers);
			...
			<td class="content_bg" valign="top"><?php echo $messageBox; ?> <input class="ui-state-default ui-corner-all"
				...
				value="Request Supervisor Approval"> <?php
				$checkUserHasEmployees = $loggedUser->GetEmployees();
				if(isset($checkUserHasEmployees))
				{
					echo "<input class=\"ui-state-default ui-corner-all\" type=\"submit\" name=\"approve\" value=\"Approve Selected Leaves\"> <input class=\"ui-state-default ui-corner-all\" type=\"submit\" name=\"notApprove\" value=\"Do Not Approve Selected Leaves\">";
				}
